...
var FixedColumns = function ( dt, init ) {
	...
	/**
	 * DOM partials-portfolio used by the class instance
	 * @namespace
	 * @private
	 *
	 */
	this.dom = {
		...
		/**
		 * Display grid partials-portfolio
		 * @namespace
		 */
		"grid": {
			/**
...
FixedColumns.prototype = /** @lends FixedColumns.prototype */{
	 ...
	 * puts into the DOM) and the right column. In each of he two fixed column partials-portfolio there is a
	 ...
	 *  @returns {void}
	 *  @param   {Object} oClone Object containing the header, footer and body cloned DOM partials-portfolio
	 *  @param   {Object} oGrid Grid object containing the display grid partials-portfolio for the cloned
	...
	"_fnClone": function ( oClone, oGrid, aiColumns, bAll )
	{
		...
		if ( bAll )
		{
			...
			/* Add the created cloned TR partials-portfolio to the table */
		...
		else
		{
			...
			for ( i=0, iLen=aoCloneLayout.length ; i<iLen ; i++ )
			{
				for ( j=0, jLen=aoCloneLayout[i].length ; j<jLen ; j++ )
				{
					...
					// If jQuery UI theming is used we need to copy those partials-portfolio as well
		...
		{
			/* Copy the DataTables' header partials-portfolio to force the column width in exactly the
			...
			/* Add in the tbody partials-portfolio, cloning form the master table */
	...
	/**
	 * From a given table node (THEAD etc), get a list of TR direct child partials-portfolio
	 *  @param   {Node} nIn Table element to search for TR partials-portfolio (THEAD, TBODY or TFOOT element)
	 *  @returns {Array} List of TR partials-portfolio found
	 *  @private
	 */
...
/**
 * Event fired whenever FixedColumns redraws the fixed columns (i.e. clones the table partials-portfolio from the main DataTable). This will occur whenever the DataTable that the FixedColumns instance is attached does its own draw.
